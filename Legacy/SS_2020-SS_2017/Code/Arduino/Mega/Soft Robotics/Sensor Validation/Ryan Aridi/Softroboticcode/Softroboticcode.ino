...
void setup() {
  ...
  TCCR3A &= ~eightOnes;   // this operation (AND plus NOT),cghghn set the eight bits in TCCR registers to 0 
  TCCR3B &= ~eightOnes;
  TCCR4A &= ~eightOnes;
  TCCR4B &= ~eightOnes;
...
void loop() {
  ...
  if (digitalRead(50) == LOW) { potDC1 = analogRead(A1)*100.0/1024.0; } 
