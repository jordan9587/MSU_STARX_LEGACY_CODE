...
// Pressure Sensor A calibration 10% to 90%
...
// Pressure Sensor B calibration, 5% to 95%
...
void loop()
{
  float analog_val = digitalRead(pressure_sensor_pin);    // read the input pin
  /* 0v (zero bars) to 10v (100 bars). Alternatively, you could maintain 1 decimal place of accuracy in bar
  if you keep with int values. */
  float map_pressure = map(analog_val, 0, ANALOG_UNITS, 0, 100);    
  // Failed: float voltage_resolution = analog_val * (ARDUINO_VOLTAGE / ANALOG_UNITS);    // Voltage
  // This was obtained by seeing that the maximum reading from AnalogRead is 4095,
  // which corresponds to 3.3V on A0 (and which also means 10V output from the sensor.) 
  // If you read 4095 from A0, it means you are measuring 100bar: 
