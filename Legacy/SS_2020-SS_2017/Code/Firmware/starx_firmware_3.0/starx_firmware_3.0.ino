 ...
 */
const int ADC_NUM = 12; // Number of Analog Digital Converted (ADC) pins available
...
void sendADC() {
    int pin_adc[ADC_NUM]; // array set to size ADC_NUM = 12.
    String message = "";
    for (int i = 0; i < ADC_NUM; i++) { // Loops ADC_NUM = 12 times.
        pin_adc[i] = analogRead(i); // Finds analogRead(0-12) and stores in pin_adc[0-12].
        message += String(pin_adc[i]); // Converts int pin_adc to string and store in message.
        if (i < ADC_NUM - 1) {
            message += PAC_DEL; // Add packet delimiter as second last element of string message.
        }
        else {
            message += PAC_TER; // Add Packet terminator as last element of string message.
        }
    }
...
void setPWM(String parameter[]) {
    int duty_cycle[4]; // sets duty_cycle to size 4.
    String message = "";
    for (int i = 0; i < sizeof(duty_cycle) / sizeof(int); i++) { //sizeof(int) == 2.
